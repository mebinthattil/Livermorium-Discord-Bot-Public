...
async def on_message(message):
    ...
    if message.content.startswith('!attendance'):
        ...
        if (is_time_between(time(18,00), time(19,00) or valid_time_check ) and (not is_attendance_given_today())) : #corresponds to 6PM to 7PM,, also time changed to 18:00 to 19:00 --(KEVAL P.)
            mycursor = mydb.cursor()
            mycursor.execute(f"SELECT * FROM user_data WHERE userID = {message.author.id}")
            result = mycursor.fetchone()
            ...
            if result == None: #checking if user is registered
                ...
                def appended_data():
                        ...
                        if attendance_counter == []: # changed to list cause it is a list type -- (KEVAL P.) 
                            ...
                            attendance_counter = int(attendance_counter[0][0]) # This attendance_counter is [(1,)] so converted into integer --(KEVAL P.) 
                        mycursor.close()
